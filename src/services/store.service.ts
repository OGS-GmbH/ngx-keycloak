...
export class KeycloakStoreService {
  ...
  public clearEmail (): void {
    ...
    const currentTokens: KeycloakTokensResponse | null = this.tokens;
   ...
   * @param email - The email, that should be saved. If `null`, the email will be removed
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
   ...
   * @returns The existing email, or `null` if not existing
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
   ...
   * @param tokens - Keycloak Tokens, that should be saved
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
  public set tokens (tokens: KeycloakTokensResponse | null) {
   ...
   * @returns The Keycloak Tokens from Storage
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
  public get tokens (): KeycloakTokensResponse | null {
    const storageValue: string | null = this._config.storageType.getItem(this._config.storageKey);

    if (storageValue === null) return null;

    return this._config.useEmailAsCurrentUser ? (JSON.parse(storageValue) as MultiKeycloakStorage).tokens ?? null : JSON.parse(storageValue) as KeycloakTokensResponse | null;
  }
   ...
   * @returns Keycloak Refresh Token
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
   ...
   * @returns Seconds of the TTL of Keycloak Access Token
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
   ...
   * @returns The decoded Access Token, or `null` if not existing
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
   ...
   * @returns The decoded Refresh Token, or `null` if not existing
   *
   * @author Simon Kovtyk
   * @since 1.0.0
   */
